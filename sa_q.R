minvalue=1800 #NEED TO BE DEFINED
maxvalue=3000 #NEED TO BE DEFINED
ymax=100 #
stepchange=10
...
q<-matrix(double(1),nrow=length(step)) #NEED TO BE DEFINED
...
for (j in 1: length(step)){
  for (i in 1:pumps){
    ...
    q[j]<-step[j] #NEED TO BE DEFINED
    
    etasa[j,i]<-waterpower(h,q[j])/epower(p,e) #NEED TO BE DEFINED
  }
  
}

hello<-q #NEED TO BE DEFINED
...
for (i in 1: pumps){
  ...
  points(data[i,3],eta[i],pch=23,bg="red",lwd=1) #NEED TO BE DEFINED
...
mtext(expression(paste("Parameter ", "Q")),side=1,col="black",line=2.5) 
